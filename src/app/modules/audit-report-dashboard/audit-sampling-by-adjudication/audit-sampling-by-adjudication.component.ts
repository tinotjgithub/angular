...
@Component({
  selector: "app-audit-sampling-by-adjudication",
  templateUrl: "./audit-sampling-by-adjudication.component.html"
})
export class AuditSamplingByAdjudicationComponent implements OnInit, OnDestroy {
  @Input() resetFormSubject: Subject<boolean> = new Subject<boolean>();
  reportSubscription: Subscription = new Subscription();
  private statusSubscription: Subscription = new Subscription();
  auditClaimCountDto: any;
  ...
  public myColumnNamesStatus = ["Auto", "Manual", "Adjusted"];
  ...
  getDataChart() {
    ...
    let responseValue = [];
    responseValue = this.auditClaimCountDto.auditClaimCountDto;
  ...
  getStatusDays() {
    ...
    this.auditReportDashboardService.getAuditSamplingAdj(
      this.datePipe.transform(fromDateValue, "yyyy-MM-dd"),
      this.datePipe.transform(toDateValue, "yyyy-MM-dd")
    );
    this.auditClaimCountDto = this.auditReportDashboardService.samplingAdjResponse;
    this.auditClaimCountDto = null;
    this.statusSubscription = this.auditReportDashboardService
      .getAuditSamplingAdjListner()
      .subscribe((data: any) => {
        this.auditClaimCountDto = data;
        this.dataStatus = [];
        if (
          this.auditClaimCountDto &&
          this.auditClaimCountDto.auditClaimCountDto &&
          this.auditClaimCountDto.auditClaimCountDto.length > 0
        ) {
          this.isDataPresent = true;
          this.getDataChart();
        } else {
    ...
    if (
      this.auditClaimCountDto === null ||
      this.auditClaimCountDto === undefined
    ) {
      this.isDataPresent = false;
      this.getNoDataChart();
    }
