...
import { AuditCountByWorkCategoryComponent } from "./audit-count-by-work-category.component";
...
import { auditTransCat } from "src/app/mocks/dashboard-mock-data";
...
class MockEnrollmentLeadDashboardService extends EnrollmentLeadDashboardService {
  auditTransCat() {
    const statusArray: any = auditTransCat;
    this.prodVolFetch.next(statusArray);
  }
}

describe("AuditCountByWorkCategoryComponent", () => {
  let component: AuditCountByWorkCategoryComponent;
  let fixture: ComponentFixture<AuditCountByWorkCategoryComponent>;
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      declarations: [AuditCountByWorkCategoryComponent],
      imports: [
        GoogleChartsModule,
        FormsModule,
        ReactiveFormsModule,
  ...
  beforeEach(() => {
    fixture = TestBed.createComponent(AuditCountByWorkCategoryComponent);
  ...
  describe("get and set values", () => {
    ...
    it("should set pie slice text in chart to a default value when data is empty", () => {
      component.userStatusDto = auditTransCat;
  ...
  describe("should fetch data from service", () => {
    ...
    it("should call service to fetch status-data", () => {
      ...
      defaultDateRange.push(todaysDate);
      component.statusDates.get("dateRange").setValue(defaultDateRange);
      const servSpy = spyOn(service, "auditTransCat");
      fixture.detectChanges();
    ...
    it("should data in chart as 'No Data' when error occurs", () => {
      ...
      defaultDateRange.push(todaysDate);
      component.statusDates.get("dateRange").setValue(defaultDateRange);
      spyOn(service, "auditTransCat").and.returnValue(
    ...
    it("should data in chart as 'No Data' when null response", () => {
      ...
      defaultDateRange.push(todaysDate);
      component.statusDates.get("dateRange").setValue(defaultDateRange);
      spyOn(service, "auditTransCat").and.returnValue(null);
      const locSpy = spyOn(component, "getNoDataChart");
      fixture.detectChanges();
      component.getStatusDays();
      expect(locSpy).toHaveBeenCalled();
    ...
    it("should map response values", () => {
      ...
      dataStatusArray.push({ status: "Address Change", claimCount: 10 });
      dataStatusArray.push({ status: "Name Correction", claimCount: 43 });
      dataStatusArray.push({ status: "Gender Correction", claimCount: 30 });
      dataStatusArray.push({ status: "Plan Correction", claimCount: 20 });
      dataStatusArray.push({ status: "Demographic Change", claimCount: 15 });
      dataStatusArray.push({ status: "Address Change", claimCount: 10 });
      component.mapResponseValue(dataStatusArray, dataStatusArray);
