...
@Component({
  selector: "app-auditor-working-time",
  templateUrl: "./auditor-working-time.component.html",
  styleUrls: ["./auditor-working-time.component.css"]
})
export class AuditorWorkingTimeComponent implements OnInit, OnDestroy {
  ...
  openPopUp(e) {
    ...
    if (e.length > 0) {
      ...
      this.editUser = { type: "auditor-working-time", status: status, count: count };
      this.editMode = true;
    }
  }
  ...
  getAuditDays() {
    ...
    const freq =
      this.frequency === "Daily"
        ...
        ? "weekly"
        : "monthly";
    this.enrollmentLeadDashboardService.getAuditorWorkingClaims(
      this.datePipe.transform(fromDateValue, "yyyy-MM-dd"),
      this.datePipe.transform(toDateValue, "yyyy-MM-dd"),
      freq
    );
    this.auditSummaryDtos = this.enrollmentLeadDashboardService.auditorWorkingClaimsResponse;
    ...
    this.auditSubscription = this.enrollmentLeadDashboardService
      ...
      .subscribe((data: any) => {
        ...
        this.dataAudit = [];
        this.auditSummaryDtos && this.auditSummaryDtos.auditSummaryDtos
